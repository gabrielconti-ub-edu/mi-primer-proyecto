// Simulación de API REST con array en memoria
let clientes = [];
let nextId = 1;
  ...
  apiGetClientes().then(data => {
    ...
    data.forEach(cliente => {
      ...
      row.innerHTML = `
        ...
        <td>
          <button class="editar" onclick="editarCliente(${cliente.CLIENTE_ID})">Editar</button>
          <button class="eliminar" onclick="eliminarCliente(${cliente.CLIENTE_ID})">Eliminar</button>
        </td>
      `;
...
form.addEventListener("submit", function (e) {
  e.preventDefault();
  const id = parseInt(document.getElementById("cliente_id").value);
  const cliente = {
  ...
  if (id) {
    apiActualizarCliente(id, cliente).then(renderClientes);
  } else {
    apiCrearCliente(cliente).then(renderClientes);
  }
  form.reset();
});
...
function eliminarCliente(id) {
  if (confirm("¿Seguro que quieres eliminar este cliente?")) {
    apiEliminarCliente(id).then(renderClientes);
  }
}
